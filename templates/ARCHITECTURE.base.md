...
## 📂 Directory Structure
    ...
    ├── build/                    <-- ⚙️ Immutable DevBox Infrastructure (Git Submodule/Template)
    ├── src/                      <-- 💻 Application Code
    │   ├── frontend/             <-- Frontend services (e.g., React, Vue)
    │   └── backend/              <-- Backend services (e.g., Python, Node, Go)
    │
...
This project runs inside isolated Docker containers. **Do not install Node, Python, or databases on your host machine.**
...
* `devbox-run frontend npm install`
* `devbox-run backend pytest`
